...
class MonitoreoclimaticoController extends Zend_Controller_Action {
	/**
...
//		$fn[] = array('$( "#tabs_hidro" ).tabs({ collapsible: true });');
	...
	public function tablaestacioncriteriovalorAction() {
		...
		for($i = 0 ; $i<count($datos) ; $i++){
			$cabecera[] = array($datos[$i][5],$datos[$i][6],$datos[$i][8],$datos[$i][12],$datos[$i][13]);
			$cabecera_grupos[] = array($datos[$i][6],0 , $datos[$i][12]);				
		}
		
//		$cabecera = array_values(array_unique($cabecera,SORT_REGULAR));
...
//		$cabecera_grupos = array_values(array_unique($cabecera_grupos,SORT_REGULAR));	
		...
		$lateral_grupo = array();
		for($i = 0 ; $i<count($datos) ; $i++){
			$lateral[] = array($datos[$i][1],$datos[$i][2],$datos[$i][1].'_'.$datos[$i][3].'_'.str_replace(':','',$datos[$i][11]),$datos[$i][11],$datos[$i][14],$datos[$i][16],$datos[$i][0]);		
			$lateral_grupo[] = array($datos[$i][1],0);		
		}
//		$lateral = array_values(array_unique($lateral,SORT_REGULAR));
//		$lateral_grupo = array_values(array_unique($lateral_grupo,SORT_REGULAR));
		$lateral = array_values($this->view->util()->multi_array_unique($lateral));	
//		$lateral_grupo = array_values($this->view->util()->multi_array_unique($lateral_grupo));	
		$lateral = $this->view->util()->ordenar_array($lateral, 5, SORT_ASC, 4, SORT_ASC, 0, SORT_ASC, 1, SORT_ASC, 3, SORT_ASC );
//		sort($lateral_grupo);
		
//		for($i = 0 ; $i<count($lateral_grupo) ; $i++){
//			for($j = 0 ; $j<count($lateral) ; $j++){
//				if($lateral_grupo[$i][0] == $lateral[$j][0])
//					$lateral_grupo[$i][1] = $lateral_grupo[$i][1] + 1;			
//			}				
//		}		
...
//				if($cant_row_group == 0){
//					for($j = 0 ; $j<count($lateral_grupo) ; $j++){					
//						if($lateral_grupo[$j][0] == $lateral[$i][0]){
//							$cant_row_group = $lateral_grupo[$j][1];
//						}						
//					}
//					$tbl_cuerpo .= '<td rowspan="'.$cant_row_group.'" valign="middle">'.$lateral[$i][0].'</td>';
//				}
