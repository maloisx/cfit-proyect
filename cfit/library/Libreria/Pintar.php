...
class Libreria_Pintar {
    ...
    public function PintarValor($valores) {
        ...
        if (is_array($valores)) {
            ...
            $script = ($this->script) ? "<script type='text/javascript'>" : "";
...
public function ComponenteSoloLectura($valores) {
        ...
        if (is_array($valores)) {
            ...
            for ($i = 0; $i < count($valores); $i++) {
             	if ($valores[$i][1] == true) {
                    ...
                    $script .= "\n\t$('#" . $valores[$i][0] . "').attr('disabled', 'disabled');";
                    $script .= "\n\t$('#" . $valores[$i][0] . "').addClass('ui-disabled-text');";
                } else if ($valores[$i][1] == false) {
                    $script .= "\n\t$('#" . $valores[$i][0] . "').attr('readonly', false);";
                    $script .= "\n\t$('#" . $valores[$i][0] . "').removeAttr('disabled');";
                    $script .= "\n\t$('#" . $valores[$i][0] . "').removeClass('ui-disabled-text');";
                }
            }
            $script .= ($this->script) ? "\n</script>\n" : "";
        }
    ...
    public function ContenidoCombo($opciones, $valor_defecto) {
        ...
        if (is_array($opciones)) {
            ...
            $html .= "<option value=\"99999\">Seleccionar...</option>";
